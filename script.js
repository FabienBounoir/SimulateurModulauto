...
function calculeTime() {
    ...
    let jourglissant = Math.round((finDate - debutDate) / (1000 * 60 * 60));
    if (jourglissant <= 24) {

        if (jourglissant < 10) {
            prixHour = data[formule][categorie].heure * jourglissant;
        }
        else {
            prixHour = data[formule][categorie].heure * 10;
        }
    }

    console.log("%cSimulateur Version 1.0.1", "color: green; font-size: 20px")
    console.log("%cDev By BadbounsTV", "color: red; font-size: 10px")
